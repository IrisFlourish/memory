<template>
  <div class="catalog">
        ...
        <div v-if="open" class="cal-pop">
          ...
          <div class="cal-months">
            <button
              ...
              class="mbtn"
              :class="{
                active: currentYM === ymKey(viewYear, m),
                has: hasPost(viewYear, m)
              }"
              :disabled="!hasPost(viewYear, m)"
              @click="pick(viewYear, m)"
            >
              <span class="mnum">{{ m }} 月</span>
              <span v-if="countOf(viewYear, m)" class="mcount">
                {{ countOf(viewYear, m) }}
              </span>
            </button>
          </div>
    ...
    <ul class="post-list">
      <li v-for="post in filtered" :key="post.url" class="post-card">
        <a :href="withBase(post.url)">
          <div class="post-title">{{ post.title }}</div>
          <div class="post-meta">
            <span v-if="post.date">{{ post.date }}</span>
            <span v-if="post.words"> · {{ post.words }} 字</span>
          </div>
          <div v-if="post.excerpt" class="post-excerpt">{{ post.excerpt }}</div>
        </a>
      </li>
      <li v-if="!filtered.length" class="empty">这段时间还没有日记。</li>
    </ul>
  </div>
</template>
...
const currentYM = ref('all')                     // 'all' | 'YYYY-MM'
const viewYear = ref(maxYear.value)              // 面板当前查看的年份
...
function hasPost(y, m) { return !!countMap.value[ymKey(y, m)] }
function countOf(y, m) { return countMap.value[ymKey(y, m)] || 0 }

function pick(y, m) {
  if (y === 'all') currentYM.value = 'all'
  else currentYM.value = ymKey(y, m)
  open.value = false
}
...
.catalog-header {
  ...
  border-bottom: 1px solid rgba(160, 130, 96, 0.15);
}
.catalog-title {
...
.mbtn.has {
  color: #5a4a34;
  cursor: pointer;
  background: rgba(201, 138, 91, 0.06);
}
.mbtn.has:hover { background: rgba(201, 138, 91, 0.18); }
.mbtn.active {
  background: #c98a5b !important;
  color: #fff !important;
  border-color: #c98a5b;
}
.mnum { display: block; }
...
.mbtn.active .mcount { opacity: 0.9; }
...
.post-card a:hover { border-color: #c98a5b; transform: translateY(-1px); }
.post-title {
  font-family: "Kaiti SC", serif;
  font-size: 1.2em;
  color: #8a3a2a;
...
.post-meta { font-family: "STFangsong", serif; font-size: 0.82em; color: var(--vp-c-text-3); }
.post-excerpt {
  margin-top: 6px;
  font-size: 0.9em;
...
.empty { padding: 2em 0; text-align: center; color: var(--vp-c-text-3); }
</style>